...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:pxs="http://xml.prexens.com/" xmlns:rs="urn:schemas-microsoft-com:rowset" xmlns:z="#RowsetSchema">
	...
	<xsl:template match="/Hoozin">
		...
		<div id="{$webpart_id}_content">
			...
			<div class="kpiWrapper" id="connectedUsersLive">

				<div class="kpiHeader">
					<span class="kpiIcon"></span>
					<h2>
						<xsl:value-of select="pxs:GetResource('Prexens.Hoozin.Sharepoint', 'AdminHome_ConnectionsDuringTheDay')"/>
					</h2>
					
				</div>

				<div class="kpiContent highchartsWrapper" id="connectedUsers">
					...
					<xsl:with-param name="sub-title" select="$sub-title"/>-->
							<xsl:with-param name="container" select="'connectedUsers'"/>
							...
							<xsl:with-param name="margin-right"  select="5"/>
							...
							<xsl:with-param name="legend-layout" select="'horizontal'"/>
							...
							<xsl:with-param name="tooltip-formatter" select="'hoozin.KPI.timeTooltipFormatter'"/>
