...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:pxs="http://xml.prexens.com/" xmlns:rs="urn:schemas-microsoft-com:rowset" xmlns:z="#RowsetSchema">
	...
	<xsl:template match="/Hoozin">
		...
		<xsl:variable name="total-connection" >
			<xsl:choose>
		...
		<xsl:variable name="connection-day-average" >
			<xsl:choose>
				<xsl:when test="sum(/Hoozin/NewDataSet/*/value) div count(/Hoozin/NewDataSet/*) != ''">
					<xsl:value-of select="sum(/Hoozin/NewDataSet/*/value) div count(/Hoozin/NewDataSet/*)"/>
				</xsl:when>
				<xsl:otherwise>0</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<div id="{$webpart_id}_content">
			...
			<div class="kpiWrapper topper" id="{$register}" data-toppers-count="10">
				...
				<div class="kpiHeader">
					...
					<h2><xsl:value-of select="pxs:GetResource('Prexens.Hoozin.Sharepoint', 'AdminReportingContent_UniqueVisits')"/></h2>
				</div>

				<div class="kpiContent">
					<xsl:choose>
						<xsl:when test="$total-connection != 0">
							<div class="theNumberWrapper">
								<div class="theNumber" style="text-align:left;">
									<span><xsl:value-of select="round($connection-day-average)"/></span><xsl:value-of select="$sufix"/>
								</div>
								<div class="theChapo" style="text-align:left; padding-bottom:10px">
									<xsl:value-of select="pxs:GetResource('Prexens.Hoozin.Sharepoint', 'UniqueVisitsTopperZone_Explanation')"/>  <br/><br/>
									<xsl:value-of select="pxs:GetResource('Prexens.Hoozin.Sharepoint', 'UniqueVisitsTopperZone_ExplanationDetail')"/> <xsl:value-of select="pxs:FormatDateTime(./NewDataSet/*[1]/XAxis, pxs:GetResource('Prexens.Hoozin.Sharepoint', 'UniqueVisitsTopperZone_DateFormat'))" />
								</div>
							</div>
							<div class="clear">
							</div>
